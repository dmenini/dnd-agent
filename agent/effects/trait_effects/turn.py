...
def cannot_move_effect(target: CharacterBase) -> None:
    ...
    target.log_event(f"{target.name} cannot move this turn.", event_type=EventType.DEBUG)


def cannot_act_effect(target: CharacterBase) -> None:
    target.action_economy.can_act = False
    target.log_event(f"{target.name} cannot act this turn.", event_type=EventType.DEBUG)
...
def extra_actions_effect(target: CharacterBase, extensions: list[ActionExtension]) -> None:
    ...
    target.log_event(f"{target.name} gains {len(extensions)} extra actions.", event_type=EventType.DEBUG)
...
def half_attacks_effect(target: CharacterBase) -> None:
    ...
    if attack_extensions[keep_count:]:
        target.log_event(f"{target.name}'s available attacks reduced to {keep_count}.", event_type=EventType.DEBUG)
