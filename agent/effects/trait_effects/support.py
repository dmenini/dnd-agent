...
def life_steal_effect(actor: CharacterBase, context: CombatContext, ratio: float) -> None:
    if context.damage:
        ...
        actor.heal(heal)
        actor.log_event(f"{actor.name} heals {heal} HP through life steal.", event_type=LogLevel.DEBUG)


def regeneration_effect(actor: CharacterBase, value: int) -> None:
    actor.heal(value)
    actor.log_event(f"{actor.name} heals {value} HP.", event_type=LogLevel.DEBUG)
